...
export default function TablesPage() {
  ...
  async function handleFileUpload(e: any) {
    ...
    try {
      ...
      const { error: insertError } = await supabase
        .from("blog")
        .insert([{ title, desc, date, slug, img: fileUrl }]);
  ...
  async function editBlog(e: any) {
    ...
    const { data, error } = await supabase
      .from("blog")
      .update({ title, desc, date, slug, img: fileUrl })
        ...
        <div className="flex flex-col gap-9 mb-6">
          <ShowcaseSection title="Blog" className="!p-6.5">
            <form action="#" onSubmit={(event) => { editOpen ? editBlog(event) : handleFileUpload(event) }}>
              ...
              {selectData?.img &&
                <div>
                  <Image src={selectData.img} alt="Blog Image" width={100} height={200} />
                </div>
              }
              <InputGroup
                label="Image"
                ...
                <TableBody>
                  {data?.map((channel, i) => (
                    ...
                    >
                      ...
                      <TableCell className="max-w-[50px] truncate">{channel.desc}</TableCell>
